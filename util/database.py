...
class ScleraSegmentationDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root: str ='data/', mode: str = "TRAIN", channel: int = 3, rotation_limit=0, elastic_transform_prob=0, train_set=None) -> None:
        self.data_dir = os.path.join(os.path.join(root, mode.upper()+'1'))
        self.mode = mode
        # scan the data directory for all files
        self.files, self.classes, self.dirs = self.scan(self.data_dir)
    ...
    def scan(self, dir) -> Tuple[List[str], List[str]]:
        ...
        for c in range(0,len(lst_dir)):
            for f in os.listdir(os.path.join(input_dir, lst_dir[c])):
                ...
                dirs.append(int(lst_dir[c]))
        return files, classes,dirs
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        input = cv2.imread(os.path.join(self.data_dir, 'input', str(c), f)) #Image.open(os.path.join(self.data_dir, 'input', str(c), f))
        ...
        if self.mode.upper() in ['TRAIN', 'VAL']:
            label =  cv2.imread(os.path.join(self.data_dir, 'label', str(c), f))  #Image.open(os.path.join(self.data_dir, 'label', str(c), f))
        ...
        return (input, np.round(label), cls, f) if self.mode.upper() in ['TRAIN', 'VAL'] else (input, cls, f)
